...
export default function MonitorViewManage() {
  const [data, setData] = useState([
    { id: 1, name: "大廳監視", layout: "4 分割", status: "啟用" },
    { id: 2, name: "停車場", layout: "6 分割", status: "停用" },
    { id: 3, name: "貨物區", layout: "單畫面", status: "啟用" },
  ]);
  ...
  const filteredData = data.filter(
    (item) => item.name.includes(search.trim())
  );
  ...
  const editItem = (item) => {
    Swal.fire({
      ...
      html: `
        ...
        <input id="layout" class="swal2-input" placeholder="版型" value="${item.layout}">
      `,
      showCancelButton: true,
      ...
      if (res.isConfirmed) {
        setData((prev) =>
          prev.map((v) =>
            v.id === item.id
              ? { ...v, name: res.value.name, layout: res.value.layout }
              : v
          )
        );
  ...
  const addItem = () => {
    Swal.fire({
      ...
      html: `
        ...
        <input id="layout" class="swal2-input" placeholder="版型">
      `,
      showCancelButton: true,
  ...
  return (
    <div className="container mt-4">
      <h3 className="mb-4 text-primary">影像監控系統 - 監控畫面管理</h3>
