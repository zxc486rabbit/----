...
export default function WiredDeviceManage() {
  const [data, setData] = useState([
    { id: 1, name: "機櫃交換機", port: "48 Port", location: "1F 機房", status: "正常" },
    { id: 2, name: "樓層集線器", port: "24 Port", location: "3F 弱電間", status: "維護中" },
  ]);
  ...
  const filteredData = data.filter(
    (item) =>
      item.name.includes(search.trim()) || item.location.includes(search.trim())
  );
  ...
  const addItem = () => {
    Swal.fire({
      ...
      html: `
        ...
        <input id="port" class="swal2-input" placeholder="Port 數">
  ...
  return (
    <div className="container mt-4">
      <h3 className="mb-4 text-primary">通訊傳輸系統 - 有線設備管理</h3>
